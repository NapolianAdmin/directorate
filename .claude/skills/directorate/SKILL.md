...
license: MIT
compatibility: Works with any coding agent that can (a) spawn a separate agent/subprocess to do bounded work and read its result, and (b) run shell commands. File placement verified against Claude Code's and OpenCode's own documented skill-loading paths; the auto-trigger itself has been confirmed in a live, independent Claude Code session (see CHANGELOG.md) — OpenCode and Codex are not yet confirmed live; for agents that only read AGENTS.md (e.g. Codex), see AGENTS.md at the repo root.
---
