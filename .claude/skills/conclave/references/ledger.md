...
This is the part that makes a conclave worth more on its tenth mission than its first.
...
`.conclave/ledger/lessons.jsonl`, recalled by tag into work orders. Tag matching is exact
...
`.conclave/STANDING-RULES.md`. These go into **every** work order regardless of tag, so
...
would affect any agent in the repo. `conclave.py lesson digest` flags candidates; you
...
Use the `skill-forge` skill in this repo to do the promotion — it handles the frontmatter,
...
```bash
conclave.py lesson digest          # regenerate LESSONS.md, surface promotion candidates
conclave.py brief                  # standing rules + top recent lessons (start of mission)
conclave.py brief --tags security  # rules + lessons for one order's domain
conclave.py lesson list --tag deploy --limit 20
```
